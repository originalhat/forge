...
class PipelinesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pipeline" do
    assert_difference('Pipeline.count') do
      post pipelines_url, params: { pipeline: { name: @pipeline.name, slug: @pipeline.slug } }
    end
  ...
  test "should update pipeline" do
    patch pipeline_url(@pipeline), params: { pipeline: { name: @pipeline.name, slug: @pipeline.slug } }
